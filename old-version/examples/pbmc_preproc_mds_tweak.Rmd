...
pbmc_10x$expr2 <- t(t(pbmc_10x$expr2) / colSums(pbmc_10x$expr2))

pbmc_10x$gene_mask <- rowSds(pbmc_10x$expr2) / rowMeans(pbmc_10x$expr2) > 1.
...
pbmc_well$expr2 <- t(t(pbmc_well$expr2) / colSums(pbmc_well$expr2))

pbmc_well$mask <- rowSds(pbmc_well$expr2) / rowMeans(pbmc_well$expr2) > 1.
...
```{r}
hist(rowSds(pbmc_10x$expr2), breaks=20)
hist(rowSds(pbmc_well$expr2), breaks=20)
```
...
hist(int$weight / sd(int$weight))
```

```{r}

int$weight <- int$weight / sd(int$weight) + 10.
```
...
```{r}
int$mds <- cmdscale(dist(t(int$expr)), k=100, eig=T)
```

```{r}
plot(int$mds$eig[1:100])
```


```{r}
int$tsne <- Rtsne(int$mds$points)
```
...
```{r}
saveRDS(list(pbmc_well, pbmc_10x, int), "../results/pbmc_preproc_mds_tweak.RDS")
```
