...
obj$normalize(normalize_factor = 1000., log_transform = TRUE)
#obj$normalized <- obj$raw
obj$combine(sd_threshold = 0.0, cv_threshold = 0.5)
dim(obj$combined)
obj$reduce(reduce_dim = 20)
obj$clean()
```
...
```{r}
set.seed(0)
obj$define_metric("primary2", "canberra")$measure("primary2")
obj$embed("primary2", "umap")
obj$plot("primary2", "embedding", "type", size=.1)
obj$plot("primary2", "embedding", "plate")
obj$plot("primary2", "embedding", "age")
``` 

```{r}
ggplot(obj$plot("primary2", "embedding", manual=TRUE)) + 
  geom_point(aes(x=x, y=y, color=type), size=.5) + 
  guides(color = guide_legend(override.aes = list(size=3.))) +
  ggtitle('Primary embedding')
```
