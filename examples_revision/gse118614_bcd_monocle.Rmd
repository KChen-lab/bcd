...
```{r}
obj <- readRDS("../output-embeddings/gse118614_bcd.RDS")
```
...
```{r}
obj@reductions$umap <- obj@reductions$umap2
```
...
png("../figures/s5a.png", width=1200, height=750, res=300)
print(VlnPlot(subobj, "monocle3_pseudotime", group.by = 'sample') + ggtitle('BCD') + NoLegend())
dev.off()
```
```{r}
...
png("../figures/s5b.png", width=1050, height=750, res=300)
print(fig)
dev.off()
fig
```