...
```{r}
obj <- RunUMAP(obj, dims = 1:30, reduction = "harmony", verbose=FALSE)
...
ggsave("sim_harmony_sample.png", height=12, width=18, dpi = 100)
```
...
DimPlot(obj, reduction = "umap", group.by = "label", label = T, repel = T, label.size = 10,  pt.size = 1) +
  theme(axis.text.x=element_blank(),
        ...
        axis.ticks.y=element_blank(),
        text=element_text(size=30))
ggsave("sim_harmony_label.png", height=12, width=18, dpi = 100)
```
```{r}
write.csv(obj@reductions$umap@cell.embeddings, '../output-embeddings/sim_eq_harmony_umap.csv')
```