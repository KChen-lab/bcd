...
```{r}
ptm = proc.time()
obj <- RunHarmony(obj, "library_id", dims.use=1:10, plot_convergence = TRUE)
proc.time() - ptm
```


```{r}
obj <- RunUMAP(obj, reduction = "harmony", dims = 1:10, verbose=FALSE)
```

```{r}
saveRDS(obj, "../output-embeddings/gse112294_lib_harmony.RDS")
```

```{r}
write.csv(obj@reductions$harmony@cell.embeddings, '../output-embeddings/gse112294_lib_harmony.csv')
write.csv(obj@reductions$umap@cell.embeddings, '../output-embeddings/gse112294_lib_harmony_umap.csv')
```
