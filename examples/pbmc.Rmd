...
pbmc_well$mask <- (pbmc_well$cell_type != "DC") & (pbmc_well$cell_type != "Removed")
sum(pbmc_well$mask)
...
```{r}
library(Matrix)
matrix_dir = "../data/raw/pbmc3k_10x/filtered_gene_bc_matrices/hg19/"
barcode.path <- paste0(matrix_dir, "barcodes.tsv")
features.path <- paste0(matrix_dir, "genes.tsv")
matrix.path <- paste0(matrix_dir, "matrix.mtx")
pbmc_10x <- list()
pbmc_10x$expr <- readMM(file = matrix.path)
rownames(pbmc_10x$expr) <- read.delim(features.path, 
                           header = FALSE,
                           stringsAsFactors = FALSE)$V1
gene_names <- read.delim(features.path, 
                           header = FALSE,
                           stringsAsFactors = FALSE)$V2
colnames(pbmc_10x$expr) = read.delim(barcode.path, 
                           header = FALSE,
                           stringsAsFactors = FALSE)$V1
```

```{r}
pbmc_10x$expr <- as.matrix(pbmc_10x$expr)
pbmc_10x$log_expr <- log2(pbmc_10x$expr + 1)


pbmc_10x$log_mean <- (rowMeans(pbmc_10x$log_expr))
sum(pbmc_10x$log_mean >= 0.5)


pbmc_10x$dispersion <- rowSds(pbmc_10x$expr) / rowMeans(pbmc_10x$expr)
pbmc_10x$dispersion[is.nan(pbmc_10x$dispersion)] <- 0

pbmc_10x$var_mask <- (pbmc_10x$log_mean > 0.5) & (pbmc_10x$dispersion > 0.5)
sum(pbmc_10x$var_mask)
```
