...
```{r}
DimPlot(obj, reduction = "umap2", group.by = "age", label = T)
```
...
```{r}
DimPlot(obj, reduction = "umap", group.by = "short_celltype", label = T, repel = T) 
```

```{r}
DimPlot(obj, reduction = "umap2", group.by = "short_celltype", label = T, repel = T) 
```
...
pca_metric <- intCriteria(obj@reductions$pca@cell.embeddings, 
                          ...
                          c("Silhouette","Calinski_Harabasz","Dunn"))
pca_metric
```

```{r}
alt_metric <- intCriteria(obj@reductions$alt@cell.embeddings / 2, 
                          as.integer(as.factor(obj$short_celltype)), 
                          c("Silhouette","Calinski_Harabasz","Dunn"))
alt_metric 
```
